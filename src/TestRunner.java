    ...
    public static void runTests(Class<?> someClass) throws Exception {
        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(BeforeSuite.class)) {
                if (!isStaticMethod(method)) throw new RuntimeException("Метод с аннотацией annotation.BeforeSuite должен быть статическим");
                if (beforeSuite == null) beforeSuite = method;
                else throw new RuntimeException("Методов с аннотацией annotation.BeforeSuite не может быть больше 1");
            }
            if (method.isAnnotationPresent(AfterSuite.class)) {
                if (!isStaticMethod(method)) throw new RuntimeException("Метод с аннотацией annotation.AfterSuite должен быть статическим");
                if (afterSuite == null) afterSuite = method;
                else throw new RuntimeException("Методов с аннотацией annotation.AfterSuite не может быть больше 1");
            }
            if (method.isAnnotationPresent(Test.class)) {
                if (isStaticMethod(method)) throw new RuntimeException("Метод с аннотацией annotation.Test НЕ должен быть статическим");
                if (!validPriorityAnnotation(method.getAnnotation(Test.class).priority())) throw new RuntimeException("Приоритет аннотации annotation.Test должен быть в пределах от 1 до 10 включительно");
            }
            if (method.isAnnotationPresent(BeforeTest.class)) {
                if (!isStaticMethod(method)) throw new RuntimeException("Метод с аннотацией annotation.BeforeTest должен быть статическим");
                beforeTest.add(method);
            }
            if (method.isAnnotationPresent(AfterTest.class)) {
                if (!isStaticMethod(method)) throw new RuntimeException("Метод с аннотацией annotation.AfterTest должен быть статическим");
        ...
        Arrays.stream(methods).filter(m -> m.isAnnotationPresent(Test.class))
                //.sorted(Comparator.comparingInt(m -> m.getAnnotation(annotation.Test.class).priority())) //если больший приоритет 1
